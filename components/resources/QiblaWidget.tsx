    ...
    if (
      ...
      try {
        ...
        if (permission === "granted") {
          ...
          setError(
            t("compassPermissionDenied") || "Pusula sensörü izni reddedildi.",
          );
        }
      } catch (err) {
        setError(
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            <h3 className="text-xl md:text-2xl font-black text-gray-800 dark:text-white flex items-center gap-2">
              {t("qiblaCompassTitle") || "Kıble Pusulası"}
              <span className="text-emerald-500 text-2xl leading-none">🕋</span>
            </h3>
            <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mt-1">
              {t("qiblaCompassDesc") || "Seccadeyi Kâbe ikonuyla hizalayın."}
            </p>
          </div>
