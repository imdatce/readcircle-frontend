...
export function StatCard({ label, value, percent, color = "gray" }: any) {
  ...
  return (
    <div className={`p-3 md:p-5 rounded-2xl md:rounded-3xl border shadow-sm text-center ${colorClasses[color] || colorClasses.gray}`}>
    ...
    <div className="flex h-screen items-center justify-center bg-transparent px-4 text-center">
      <div className="flex flex-col items-center gap-5">
        ...
        <div className="flex flex-col gap-3 items-center">
          <p className="text-gray-800 dark:text-gray-200 font-bold animate-pulse text-base md:text-lg">{t("loading")}</p>
          {isSlowLoad && (
            <p className="text-gray-500 dark:text-gray-400 text-xs md:text-sm max-w-[280px] animate-in fade-in duration-1000 leading-relaxed bg-emerald-50 dark:bg-emerald-900/10 p-3 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
              {t("serverWakingUpPart1") || "Waking up server. This process might take "}
              <span className="font-bold text-emerald-600 dark:text-emerald-400">30-40 {t("seconds") || "seconds"}</span>{" "}
              {t("serverWakingUpPart2") || "please wait without closing the tab."}
            </p>
          )}
        </div>
      </div>
...
export function NamePromptModal({ isOpen, onClose, onSubmit, tempName, setTempName, t }: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-gray-900 rounded-[2rem] p-6 md:p-8 w-full max-w-sm shadow-2xl border border-gray-100 dark:border-gray-800 animate-in zoom-in-95 duration-200">
        <div className="text-center mb-6">
          ...
          <h3 className="text-xl font-black text-gray-900 dark:text-white mb-2">{t("joinTitle") || "What is your name?"}</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">{t("joinIntro") || "Please enter your name to take the part."}</p>
        </div>
        <input type="text" value={tempName} onChange={(e) => setTempName(e.target.value)} placeholder={t("yourNamePlaceholder") || "Full Name"} className="w-full px-4 py-4 bg-gray-50 dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-xl text-lg font-bold text-center outline-none focus:border-blue-500 transition-all dark:text-white mb-4" autoFocus onKeyDown={(e) => e.key === "Enter" && onSubmit()} />
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-3 bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 rounded-xl font-bold text-sm hover:bg-gray-200 transition-colors">{t("cancel")}</button>
          <button onClick={onSubmit} disabled={!tempName.trim()} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/30 disabled:opacity-50 disabled:shadow-none">{t("continue")}</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white dark:bg-gray-900 rounded-[2.5rem] p-6 md:p-10 w-full max-w-md shadow-2xl border border-emerald-100 dark:border-emerald-900/30 animate-in zoom-in-95 duration-300 text-center relative overflow-hidden">
        ...
        <h3 className="text-2xl font-black text-gray-800 dark:text-white mb-6 uppercase tracking-wider">{t("completed") || "Tamamlandı!"}</h3>
        <p className="text-sm md:text-base text-gray-500 dark:text-gray-400 mb-6 px-2 font-medium">{t("prayerMessage") || "Allah kabul etsin. İnşallah bu ayetteki müjdeye mazhar olursun..."}</p>
        <div className="bg-emerald-50/50 dark:bg-emerald-900/10 p-5 rounded-3xl mb-8 border border-emerald-100/50 dark:border-emerald-800/30">
          <p className="font-serif text-2xl md:text-3xl text-gray-800 dark:text-gray-100 mb-4 leading-relaxed" dir="rtl" lang="ar">{completedAyah.arabic}</p>
          <p className="text-sm md:text-base text-gray-600 dark:text-gray-300 font-medium italic mb-3">&quot;{t(completedAyah.translationKey)}&quot;</p>
          <span className="inline-block px-3 py-1 bg-white dark:bg-gray-800 rounded-lg text-xs font-bold text-emerald-600 dark:text-emerald-500 shadow-sm border border-emerald-50 dark:border-emerald-900/50">{t(completedAyah.refKey)}</span>
        </div>
        <button onClick={onClose} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-lg hover:bg-emerald-700 transition-colors shadow-lg shadow-emerald-500/30 active:scale-95">{t("close") || "Kapat"}</button>
      </div>
    </div>
  );
}