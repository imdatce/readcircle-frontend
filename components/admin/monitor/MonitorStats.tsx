  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      <div className="relative bg-white dark:bg-gray-900 p-6 rounded-3xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden group">
        ...
        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">{t("total")}</p>
        <p className="text-4xl font-black text-gray-800 dark:text-white tracking-tight">{stats.total}</p>
      </div>
      ...
      <div className="relative bg-white dark:bg-gray-900 p-6 rounded-3xl border border-blue-100 dark:border-blue-900/30 shadow-sm overflow-hidden group">
        ...
        <div className="flex justify-between items-end">
          <div>
            <p className="text-[10px] font-bold text-blue-400 uppercase tracking-widest mb-2">{t("distributed")}</p>
            <p className="text-4xl font-black text-blue-600 dark:text-blue-400 tracking-tight">{stats.distributed}</p>
          </div>
          <span className="text-lg font-bold text-blue-300 dark:text-blue-500/50 mb-1">%{stats.distPercent}</span>
        </div>
        <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-4 overflow-hidden">
          <div className="h-full bg-blue-500 rounded-full" style={{ width: `${stats.distPercent}%` }}></div>
        </div>
      </div>
      ...
      <div className="relative bg-white dark:bg-gray-900 p-6 rounded-3xl border border-emerald-100 dark:border-emerald-900/30 shadow-sm overflow-hidden group">
        ...
        <div className="flex justify-between items-end">
          <div>
            <p className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest mb-2">{t("completed")}</p>
            <p className="text-4xl font-black text-emerald-600 dark:text-emerald-400 tracking-tight">{stats.completed}</p>
          </div>
          <span className="text-lg font-bold text-emerald-300 dark:text-emerald-500/50 mb-1">%{stats.compPercent}</span>
        </div>
        <div className="w-full h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-4 overflow-hidden">
          <div className="h-full bg-emerald-500 rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]" style={{ width: `${stats.compPercent}%` }}></div>
        </div>
      </div>
    </div>
  );
