        ...
        >
          ...
          {!isCompleted && (
            <circle
              ...
              className={`transition-all duration-500 ease-out ${readOnly ? "text-gray-400 dark:text-gray-600" : "text-emerald-500 dark:text-emerald-400"} dark:drop-shadow-[0_0_4px_rgba(52,211,153,0.5)]`}
            />
          )}
        </svg>

        <div
          className={`relative flex items-center justify-center rounded-full bg-gradient-to-b from-gray-100 to-gray-300 dark:from-gray-800 dark:to-black shadow-[0_10px_30px_rgba(0,0,0,0.15)] dark:shadow-[inset_0_2px_4px_rgba(255,255,255,0.1)] border border-white/50 dark:border-gray-700 z-10 ${isModal ? "w-52 h-52" : "w-36 h-36"} `}
        >
          ...
          <button
            ...
            className={`relative flex flex-col items-center justify-center rounded-full transition-all duration-200 ease-out group ${buttonSize} ${
              isCompleted
          ...
          >
            ...
            <div className="flex flex-col items-center leading-none select-none z-10">
              <span
                className={`font-mono font-bold tracking-wider transition-colors duration-300 drop-shadow-sm ${textSize} mb-1 ${isCompleted ? "text-emerald-600 dark:text-emerald-400" : readOnly ? "text-gray-400 dark:text-gray-600" : "text-white dark:text-emerald-500 dark:drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]"}`}
              >
                {currentCount}
              </span>

              <span
                className={`text-[9px] font-bold uppercase tracking-[0.2em] ${isCompleted ? "text-emerald-500 dark:text-emerald-600" : "text-emerald-100/80 dark:text-gray-500"} ${readOnly ? "text-gray-400" : ""}`}
              >
                {isCompleted
        ...
        {isCompleted ? (
          ...
          <span className="text-xs font-medium text-gray-400 dark:text-gray-600 tracking-wide animate-pulse">
            {readOnly
              ...
              : t("tapToCount") || "Saymak İçin Dokun"}
          </span>
        )}
      </div>
    </div>
