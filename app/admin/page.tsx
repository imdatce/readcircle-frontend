...
export default function AdminPage() {
    ...
    const handleCreate = async () => {
        if (selectedResources.length === 0) return alert("Lütfen en az bir kaynak seçiniz.");
    ...
    const handleResetData = async () => {
        if (!confirm("Tüm veritabanı sıfırlanıp varsayılan veriler yüklenecek. Emin misiniz?")) return;
        try {
        ...
        <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
            <div className="bg-white p-8 rounded-lg shadow-lg max-w-lg w-full">
                ...
                <div className="flex justify-between items-center mb-6 border-b pb-4">
                    <h1 className="text-2xl font-bold text-gray-800">Dağıtım Paneli</h1>
                    <button onClick={handleResetData} className="text-xs text-red-500 underline hover:text-red-700">Sıfırla</button>
                </div>
                ...
                {selectedResourcesWithInput.length > 0 && (
                    <div className="mb-6 animate-in fade-in slide-in-from-top-2 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                        <p className="block text-amber-800 font-bold mb-3 text-sm border-b border-amber-200 pb-2">
                            Hedef Sayıları Belirle (Opsiyonel)
                        </p>
                ...
                <div className="mb-6">
                    <label className="block text-gray-700 font-bold mb-2">Kaç Kişi Okuyacak?</label>
                    <input
                        type="number"
                ...
                {createdLink && (
                    <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded text-center">
                        <p className="text-green-800 font-bold mb-2">Link Oluşturuldu:</p>
