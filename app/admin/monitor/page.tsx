...
function MonitorContent() {
    ...
    const fetchData = async (codeToFetch: string) => {
        ...
        try {
            const res = await fetch(`${apiUrl}/api/distribution/get/${codeToFetch}?t=${Date.now()}`);
            ...
            {session && (
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    {Object.entries(groupedData).map(([resourceName, assignments]) => {
                        ...
                        return (
                            <div key={resourceName} className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden transition-all duration-200">
                                <button
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">{resourceName}</h3>
                                    </div>

                                    <div className="flex items-center gap-4">
                                        <div className="hidden md:flex flex-col items-end mr-4">
                                            <span className="text-xs text-gray-500 font-bold mb-1">
                                                {t('occupancy')}: <span className={percentage === 100 ? "text-green-600" : "text-blue-600"}>%{percentage}</span>
                                            </span>
                                        ...
                                        <span className={`px-3 py-1 rounded-full font-bold text-sm border ${isOpen ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-blue-600 border-blue-200'}`}>
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            <thead>
                                                <tr className="bg-slate-50 text-gray-500 text-xs uppercase tracking-wider border-b">
                                                    ...
                                                    <th className="px-6 py-3 font-semibold">{t('resource')}</th>
                                                    <th className="px-6 py-3 font-semibold">{t('assignedTo')}</th>
                                                    <th className="px-6 py-3 font-semibold text-center">{t('progress')}</th>
                                                    <th className="px-6 py-3 font-semibold text-right">{t('statusHeader')}</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-100">
                                                {assignments.map((item) => (
                                                    <tr key={item.id} className={`hover:bg-gray-50 transition ${item.isTaken ? 'bg-white' : 'bg-slate-50'}`}>
                                                        <td className="px-6 py-4 text-gray-500 font-mono text-sm">{item.participantNumber}</td>
                                                        <td className="px-6 py-4 text-gray-800 font-medium">
                                                            {item.resource.type === "JOINT" ? t('target') : (item.resource.type === "PAGED" ? t('page') : t('part')) + ` ${item.startUnit} - ${item.endUnit}`}
                                                        </td>
                                                        <td className="px-6 py-4">
                                                            {item.isTaken ? (
                                                                <div className="flex items-center gap-2">
                                                                    <div className="w-8 h-8 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-bold text-xs uppercase">{item.assignedToName?.substring(0, 2)}</div>
                                                            ...
                                                            {item.resource.type === "COUNTABLE" || item.resource.type === "JOINT" ? (
                                                                <div className="flex flex-col items-center">
                                                                    <span className="text-xs text-gray-400 font-bold mb-1">
                                                                        {t('targetLabel')}: {item.endUnit - item.startUnit + 1}
                                                                    </span>

                                                                    {(() => {
                                                                        const progressItem = item as unknown as { currentCount?: number };
                                                                        const current = progressItem.currentCount;

                                                                        const total = item.endUnit - item.startUnit + 1;

                                                                        const displayVal = (current !== undefined && current !== null) ? current : total;

                                                                        const isFinished = displayVal === 0;

                                                                        return (
                                                                            <span className={`inline-block px-3 py-1 rounded font-bold text-sm border 
                                                                                ${isFinished
                                                                                    ? "bg-green-100 text-green-700 border-green-300"
                                                                                    : "bg-amber-50 text-amber-800 border-amber-200"
                                                                                }`}>
                                                                                {displayVal}
                                                                                <span className="text-[10px] ml-1 uppercase">{t('remaining') || "KALAN"}</span>
                                                                            </span>
                                                                        );
                                                                    })()}
                                                                </div>
                                                        ...
                                                        <td className="px-6 py-4 text-right">
                                                            {item.isTaken ? (
                                                                ...
                                                                <span className="inline-flex items-center gap-1 bg-gray-100 text-gray-500 px-3 py-1 rounded-full text-xs font-bold border border-gray-200">{t('statusEmpty')}</span>
                                                            )}
                                                        </td>
                                                    </tr>
                                                ))}
