...
export default function Home() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const resJoined = await fetch(`${apiUrl}/api/distribution/my-sessions?name=${user}`);
      if (resJoined.ok) setMySessions(await resJoined.json());

      const resCreated = await fetch(`${apiUrl}/api/distribution/my-created-sessions?name=${user}`);
        ...
        {user ? (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 w-full">
            ...
            <h2 className="text-center text-2xl font-bold text-gray-700 mb-8">
              {t('welcome')} <span className="text-blue-600">{user}</span> <br />
            </h2>

            <div className="grid md:grid-cols-2 gap-6 mb-12 max-w-4xl mx-auto">
              <div onClick={() => router.push('/admin')} className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl hover:border-blue-200 transition-all cursor-pointer group transform hover:-translate-y-1">
              ...
              <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl hover:border-green-200 transition-all group transform hover:-translate-y-1">
                <div className="flex items-center gap-4 mb-4">
                  ...
                  <h3 className="text-xl font-bold text-gray-800">{t('joinDistTitle')}</h3>
                </div>
                ...
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {mySessions.map((session) => (
                      <div key={session.id} onClick={() => router.push(`/join/${session.code}`)} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center hover:shadow-md hover:border-blue-300 transition cursor-pointer group">
                        <div>
          ...
          <div className="flex flex-col items-center animate-in fade-in zoom-in duration-500">
            <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-md w-full text-center">
              ...
              <div className="flex flex-col gap-3">
                <Link href="/login" className="w-full py-3 bg-blue-600 text-white rounded-lg font-bold shadow hover:bg-blue-700 transition">{t('guestLogin')}</Link>
                <Link href="/register" className="w-full py-3 bg-white text-blue-600 border border-blue-600 rounded-lg font-bold shadow hover:bg-blue-50 transition">{t('guestRegister')}</Link>
              </div>
              <div className="mt-8 border-t pt-6">
                <p className="text-sm text-gray-500 mb-2">{t('guestCheckCode')}</p>
                <form onSubmit={handleJoin} className="flex gap-2">
                  <input type="text" placeholder={t('guestCodePlaceholder')} className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm" value={code} onChange={(e) => setCode(e.target.value)} />
                  <button className="px-4 py-2 bg-gray-800 text-white rounded text-sm font-bold">{t('joinButton')}</button>
                </form>
              </div>
            </div>
          </div>
